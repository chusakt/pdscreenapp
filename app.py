...
# --- load model ---
model_pkl_file = "Model_pickle_tremor_post_1.pkl"  
with open(model_pkl_file, 'rb') as file:  
...
def predict_questionaire_chk():
    try:
        if request.is_json:
            req = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(req['mocking'])).decode())
...
@app.route('/predict_dualtap_chk', methods=['POST'])  
def predict_dualtap_chk():
    try:
        if request.is_json:
            data = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(data['mocking'])).decode())
            if (decMessage == "thisisaoriginalstring"):
...
@app.route('/predict_tremor_rest_chk', methods=['POST'])  
def predict_tremor_rest_chk():
    try:
        if request.is_json:
            data = request.get_json()
...
@app.route('/predict_tremor_post_chk', methods=['POST'])  
def predict_tremor_post_chk():
    try:
        if request.is_json:
            data = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(data['mocking'])).decode())
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_gait_stab_chk', methods=['POST'])  
def predict_gait_stab_chk():
    try:
        if request.is_json:
            data = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(data['mocking'])).decode())
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_gait_walk_chk', methods=['POST'])  
def predict_gait_walk_chk():
    try:
        if request.is_json:
            data = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(data['mocking'])).decode())
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_pinchtosize_chk', methods=['POST'])  
def predict_pinchtosize_chk():
    try:
        if request.is_json:
            data = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(data['mocking'])).decode())
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_voice_ahh_chk', methods=['POST'])  
def predict_voice_ahh_chk():
    # create lists to put the results
    file_list = []
    duration_list = []
    mean_F0_list = []
    ...
    try:
        if request.is_json:
            jsonData = request.get_json()
            decMessage = str(fernet.decrypt(str.encode(jsonData['mocking'])).decode())
            if (decMessage == "thisisaoriginalstring"):             
                pName = jsonData["patientName"]
                voicebase64 = jsonData["data"]
                wavFilename = pName+"_temp.wav"
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_voice_ypl_chk', methods=['POST'])  
def predict_voice_ypl_chk():
    # create lists to put the results
    file_list = []
    duration_list = []
    mean_F0_list = []
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_voting_chk', methods=['POST'])  
def predict_voting_chk():
    filenamecombination = 'listof_best_combination.csv'
    # create lists to put the results
    try:
        if request.is_json:
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_voice_ahh_feature', methods=['POST'])  
def predict_voice_ahh_feature():
    # create lists to put the results
    file_list = []
    duration_list = []
    mean_F0_list = []
    ...
    try:
        if request.is_json:
            ...
            predictions_ = loaded_model_va.predict(X)        
            # return jsonify({"show something":str('this is something')}) 
            return jsonify({"feature0":X[0][0],"feature1":X[0][1],"feature2":X[0][2],"feature3":X[0][3],"feature4":X[0][4],"feature5":X[0][5],"feature6":X[0][6]}) 
    except:
        return jsonify({"prediction":str(2)})  
...
#
# +++++++++++++++++++++++++++++++++
@app.route('/predict_voice_ypl_imp', methods=['POST'])  
def predict_voice_ypl_imp():
    # create lists to put the results
    file_list = []
    duration_list = []
    mean_F0_list = []
