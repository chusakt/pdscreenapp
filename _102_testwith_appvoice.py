...
modeltouse = "trained_model_voice_YaiParLarn_SMOTE.pkl"
featuretotest = "voice_featureAppdata.csv"
...
# Filter to select only rows where 'Filename' contains the specified voice type
df = df[df['Filename'].str.contains(voice_type, na=False)]

# Remove the 'Filename' column
df = df.drop(columns=['Filename'])

# Filter out the specified columns
columns_to_remove = ['Index', 'Folder Name', 'Timestamp Year', 'Age', 'Gender', 'Birth Year', 'Duration']
...
# Select only the columns starting from 'Mean F0' to the end for features
features = filtered_df.loc[:, 'Mean F0':]
